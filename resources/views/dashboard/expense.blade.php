        ...
        <a href="{{config('app.url')}}/edit/transaction/{{$expense->id}}">
            <div class="col-sm-12 col-lg-12 transaction_card_big">
                <div class="card-counter primary">
                    <span class="col-xs-8 col-lg-6 text-light" style="padding:2px"><b>Title:</b> {{$expense->title}}</span>
                    <span class="col-xs-4 col-lg-6 text-right text-light" style="padding:2px">{{$expense->currency->code}} <strong>{{$expense->amount}}</strong></span>
                    <span class="col-xs-8 col-lg-6 text-light" style="padding:2px"><b>Categ:</b> {{$expense->category->title}}</span>                        
                    <span class="col-xs-4 col-lg-6 text-right text-light" style="padding:2px"> % {{round($expense->percentage,0)}}</span>
                    <span class="col-xs-7 col-lg-6 text-light" style="padding:2px"><b>Date:</b> {{$expense->start_date}}</span>
                    <span class="col-xs-5 col-lg-6 text-right {{$expense->category->logo->class_name}}" style="padding:2px"></span>
                </div>
            </div>
        </a>
    @endforeach
